...
void HudWidget::drawHeadingTape(QPainter &p, const QRectF &r)
{
    ...
    for (int i = 0; i <= 60; i += 5) {
        ...
        if (((int)qRound(deg) % 30) == 0) {
            ...
            QFont f = p.font();
            f.setPointSizeF(r.height()*0.16*textMultiplier);
            p.setFont(f);
...
void HudWidget::drawAltitudeTape(QPainter &p, const QRectF &r)
{
    ...
    // Inner scale region
    QRectF inner = r.adjusted(r.width()*0.10, r.height()*0.08, -r.width()*0.10, -r.height()*0.08);
    ...
    // Altitude value font — sized to match the bottom readout values
    QFont f2 = p.font();
